...
export default function AddressesTab() {
  ...
  const handleAdd = async (dto) => { await add(dto); setEditing(null); };
  const handleUpdate = async (dto) => { await update(editing.id, dto); setEditing(null); };
  ...
  return (
    <>
      <section className="my-card my-section addr-list-card">
        ...
        {!loading && items.length === 0 && <div className="empty">등록된 배송지가 없습니다.</div>}
            ...
            {items.map(a => (
              <li key={a.id} className={`addr-item ${a.isDefault ? "is-default" : ""}`}>
                ...
                <div className="addr-actions">
                  {!a.isDefault && <button className="btn" onClick={() => setDefault(a.id)}>기본설정</button>}
                  <button className="btn" onClick={() => setEditing(a)}>수정</button>
                  <button className="btn danger" onClick={() => remove(a.id)}>삭제</button>
                </div>
              </li>
            ))}
          </ul>
      ...
      <section className="my-card my-section addr-form-card">
        <h3>{editing ? "배송지 수정" : "배송지 추가"}</h3>
        <AddressForm
